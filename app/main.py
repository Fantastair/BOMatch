...
def password_page(
    ...
    return TEMPLATES.TemplateResponse(
        request,
        "password.html",
        {"error": None, "ok": None, "user": user.username, "username_value": ""},
    )
...
def change_password(
    ...
    return TEMPLATES.TemplateResponse(
        ...
        {
            "error": error,
            "ok": "密码已修改" if error is None else None,
            "user": user.username,
            "username_value": "",
        },
...
@app.post("/account/username", response_model=None)
def change_username(
    request: Request,
    ...
    """修改当前登录用户的用户名（需验证当前密码，成功后刷新会话 cookie）"""
    user = get_optional_user(request, session)
    if user is None:
        return RedirectResponse("/login", status_code=303)
    ...
    resp = TEMPLATES.TemplateResponse(
        ...
        {
            "error": error,
            "ok": f"用户名已更新为 {new_username}" if error is None else None,
            "user": user.username,
    ...
    if error is None:
        # 会话 cookie 里存的是旧用户名，需刷新为新用户名，否则下次请求会掉登录
        set_session_cookie(resp, new_username)
    return resp
